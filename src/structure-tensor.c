...
{ 
  ...
  gaussian_convolution_gauss_seidel(cpy,sigma_pre,10);
  print_image_info("presmoothed",cpy);
  write_pgm_image("/home/urp/Documents/dev/dilate/test/presmoothed.pgm",cpy);
...
/*  #pragma omp parallel shared(cpy, \
  ...
  gaussian_convolution_gauss_seidel(stxx,sigma,50);
  gaussian_convolution_gauss_seidel(styy,sigma,50);
  gaussian_convolution_gauss_seidel(stxy,sigma,50);
